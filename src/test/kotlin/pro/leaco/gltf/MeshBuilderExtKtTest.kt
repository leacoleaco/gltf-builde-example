...
class MeshBuilderExtKtTest {
    ...
    fun testAddGeometryPlaneTrans() {
        ...
        val node = builder.buildNodeWith(
            MeshBuilder("face-XY1")
                .withMaterial(kineticaMaterial)
                .addGeometryPlane(geom, true)
        )

        node.rotation = floatArrayOf(0.7f, 0f, 0.7f, 0f)


        val outFile: File = TestShapeModels.getFile("test_geom_transform")
        builder.writeGltf(outFile)
